...
#'
#' This function checks if a rook at a given position on a chess board has any
#' legal moves.
#'
...
#'   `can_rook_move()` is an alias for `rook_can_move()`.
#'
...
#'   rook on the board. The first element is the row number and the second
#'   element is the column number.
#' @param color An optional character vector giving the color of the rook.
#'   Defaults to the color of the player whose turn it is to move.
#'
#' @return A logical value indicating whether the rook has any legal moves
#'   (TRUE) or not (FALSE).
#' @examples
...
#' @export
can_rook_move <- function(board, coordinates, color = board$to_move) {
  # Validate input
  ...
  # Define the directions a rook can move
  directions <- list(c(-1, 0), c(0, -1), c(0, 1), c(1, 0))
  ...
  for (direction in directions) {
    ...
    # Make a copy of the board and move the rook to the new coordinates
...
#' @rdname can_rook_move
#' @export
rook_can_move <- function(board, coordinates, color = board$to_move) {
  can_rook_move(board, coordinates, color)
}
